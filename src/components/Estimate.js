...
export default function Estimate(props) {
    ...
    const previousQuestion = () => {
    	const newQuestions = cloneDeep(questions);
    	//currentyActive holds single question which has active property to true
    	const currentlyActive = newQuestions.filter(question => question.active);
    	const activeIndex = currentlyActive[0].id - 1 ;
    	const nextIndex = activeIndex - 1 ;
    ...
    const handleSelect = (id) => {
    	...
    	switch (currentlyActive[0].subtitle) {
    		case 'Select one.' : if (previousSelected[0]) {
    									previousSelected[0].selected = !previousSelected[0].selected
    								}
    								newSelected.selected = !newSelected.selected;
    								break;
    			 default       :  	newSelected.selected = !newSelected.selected;
    			 					break;				
    	}

    	switch (newSelected.title) {
    		case 'Custom Software Development' : setQuestions(softwareQuestions);
    											 ...
    											 setUsers("");
    											  break;
		    case 'iOS/Android Development'	  : setQuestions(softwareQuestions);
		    									setService(newSelected.title);
		    									setPlatforms([]);
    											 setFeatures([]);
    											 setCustomFeatures("");
    											 ...
    											 setUsers("");
		 										 break;
		    case 'Website Development' 		   : setQuestions(websiteQuestions);
		    									 setService(newSelected.title);
		    									 setPlatforms([]);
    											 setFeatures([]);
    											 setCustomFeatures("");
    											 ...
    											 setUsers("");
		  										break;
		  							default		: setQuestions(newQuestions);
		  											break;
    	}
    	}
    	...
    	const onChange = event => {
			...
			const getTotal = () => {
				...
				//This returns an array filled with all objects for each option
				//in any question we have selected
			...
			const getCategory = () => {
				if (questions.length === 2 ) 
				{
					const newCategory = questions.filter(question => question.title === 
						"Which type of website are you wanting?")[0].options.filter(option => 
			...
			const saveEstimateToDb = async (estimateDetails) => {
				console.log(" Step 11 " + estimateDetails.name) ;
				console.log(" Step 11 " + estimateDetails.email) ;
				console.log(" Step 11 " + estimateDetails.phone) ;
				console.log(" Step 11 " + estimateDetails.total) ;
				console.log(" Step 11 " + estimateDetails.category) ;
				console.log("saveEstimateToDb called = ");
					try {
				const estimateModel = new EstimateModel(estimateDetails)
						await estimateModel.save()
						console.log("Save Successfull Estimate");		
					} catch (e) {
						console.log("Save Failed Estimate");	
					}
			}
    ...
    return (
    		<Grid container direction="row" >
    			...
    			<Grid item container direction="column" lg alignItems="center" style={{marginRight : matchesMD ? 0 : "2em" , marginBottom : "25em"}}>
    					...
    					<React.Fragment key={index}>
    							<Grid item >
			    					...
			    					style={{fontWeight : 500  , fontSize : "2.25rem" , marginTop : "5em" , lineHeight : 1.25 ,
			    				     marginLeft : matchesSM ? "1em" : 0 , marginRight : matchesSM ? "1em" : 0}} >
			    					  {question.title}
			    					</Typography>
								...
								<Grid item container>
								 {question.options.map((option , index) => (
									<Grid item key={index} container direction="column" md 
									component={Button} 
									style={
										   {
											display : "grid" ,
    				...
    				<Grid item >
					    <Button variant="contained" className={classes.estimateButton} disabled={estimateDisabled()} 
					    onClick={() => {
					    	setDialog(true); 
					    	getTotal(); 
					    	getPlatforms();
    			...
    			<Dialog open={dialogOpen} onClose={() => setDialog(false)} fullWidth maxWidth="lg" 
    			fullScreen={matchesSM} style={{zIndex : 1302}}>
    				...
    				<DialogContent>
    					...
    					alignItems={matchesSM ? "center" : undefined}>
    						<Grid item container direction="column" md={7} style={{maxWidth : "20em"}}>
									...
									<Grid item >
										<Typography variant="body1" align={matchesSM ? "center" : undefined} paragraph style={{lineHeight : 1.25}}>
										...
										<Typography variant="body1" align={matchesSM ? "center" : undefined} paragraph >
											Fill out your name , phone , number and email , place your request and we will 
											get back to you with details moving forward and a final price.
										</Typography>
									</Grid>
							</Grid>
							...
							alignItems={matchesSM ? "center" : undefined}>
								...
								<Grid item>
									...
									onClick={sendEstimate} disabled={name.length === 0 || message.length === 0 || phoneHelper.length !== 0 || emailHelper.length !== 0 }>
								...
								<Grid item style={{marginBottom : matchesSM ? "5em" : 0}}>
									<Button style={{fontWeight : 300}} color="primary" onClick={() => setDialog(false)}>
									Cancel
									</Button>
								</Grid>
								</Hidden>
